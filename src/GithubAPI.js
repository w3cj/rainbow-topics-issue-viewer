...
export default {
  getAllIssues(use_cache = true, statusCallback) {
    if (use_cache && cache) return Promise.resolve(cache);
    let allIssues = [];
    return this.getIssues(1).then(async ({ result, lastPage }) => {
      allIssues = allIssues.concat(result);
      let currentPage = 2;
      while (currentPage <= lastPage) {
        statusCallback(
          `Retrieved ${currentPage - 1} out of ${lastPage} pages of issues.`
        );
        let result;
        try {
          result = await this.getIssues(currentPage);
        } catch (error) {
          statusCallback(error.message);
          return Promise.reject(error);
        }
        allIssues = allIssues.concat(result.result);
        currentPage++;
      }
      cache = allIssues;
...
async function get(url) {
  url = localStorage.GITHUB_ACCESS_TOKEN
    ? `${url}&access_token=${localStorage.GITHUB_ACCESS_TOKEN}`
    : url;
